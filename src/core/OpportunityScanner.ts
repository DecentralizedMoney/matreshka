    ...
    for (const symbol of strategy.symbols) {
      ...
      for (const exchangeId of strategy.exchanges) {
        try {
          ...
          this.logger.debug(`Failed to get data for ${symbol} on ${exchangeId}:`, error);
        }
      }
    ...
    for (const triangle of triangles) {
      for (const exchangeId of strategy.exchanges) {
        try {
          ...
          this.logger.debug(`Triangular arbitrage scan failed for ${exchangeId}:`, error);
        }
      }
    }
  }
      ...
      for (const spotExchange of spotExchanges) {
        for (const perpExchange of perpExchanges) {
          try {
            ...
            this.logger.debug('Funding rate arbitrage scan failed:', error);
          }
        }
      }
    }
